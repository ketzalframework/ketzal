...
pub trait FromParam: Sized {
    fn from_param(value: &str) -> Result<Self, Box<Response>>;
}

impl FromParam for i32 {
    fn from_param(v: &str) -> Result<Self, Box<Response>> {
        v.parse().map_err(|_| Box::new(HTTPException!(status_code = 400, detail = "Invalid i32",)))
    }
}

impl FromParam for i64 {
    fn from_param(v: &str) -> Result<Self, Box<Response>> {
        v.parse().map_err(|_| Box::new(HTTPException!(status_code = 400, detail = "Invalid i64",)))
    }
}

impl FromParam for u32 {
    fn from_param(v: &str) -> Result<Self, Box<Response>> {
        v.parse().map_err(|_| Box::new(HTTPException!(status_code = 400, detail = "Invalid u32",)))
    }
}

impl FromParam for u64 {
    fn from_param(v: &str) -> Result<Self, Box<Response>> {
        v.parse().map_err(|_| Box::new(HTTPException!(status_code = 400, detail = "Invalid u64",)))
    }
}
...
pub trait FromParams: Sized {
    fn from_params(params: &Params) -> Result<Self, Box<Response>>;
}

impl FromParams for () {
    fn from_params(_: &Params) -> Result<Self, Box<Response>> {
        Ok(())
    }
}
...
impl<T: FromParam> FromParams for (T,) {
    fn from_params(params: &Params) -> Result<Self, Box<Response>> {
        ...
        let value = it.next().ok_or_else(|| {
            Box::new(HTTPException!(status_code = 400, detail = "Missing param",))
        })?;
...
impl<A: FromParam, B: FromParam> FromParams for (A, B) {
    fn from_params(params: &Params) -> Result<Self, Box<Response>> {
        ...
        let a = A::from_param(it.next().ok_or_else(|| {
            Box::new(HTTPException!(status_code = 400, detail = "Missing param",))
        })?)?;

        let b = B::from_param(it.next().ok_or_else(|| {
            Box::new(HTTPException!(status_code = 400, detail = "Missing param",))
        })?)?;
...
impl<A: FromParam, B: FromParam, C: FromParam> FromParams for (A, B, C) {
    fn from_params(params: &Params) -> Result<Self, Box<Response>> {
        ...
        let c = C::from_param(it.next().ok_or_else(|| {
            Box::new(HTTPException!(status_code = 400, detail = "Missing param",))
        })?)?;
